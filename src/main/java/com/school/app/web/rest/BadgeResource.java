...
/**
 * REST controller for managing {@link com.school.app.domain.Badge}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class BadgeResource {
    ...
    public ResponseEntity<Badge> updateBadge(@PathVariable(value = "id", required = false) final Long id, @RequestBody Badge badge)
    ...
    public ResponseEntity<Badge> partialUpdateBadge(@PathVariable(value = "id", required = false) final Long id, @RequestBody Badge badge)
        throws URISyntaxException {
        ...
        Optional<Badge> result = badgeRepository
            ...
            .map(existingBadge -> {
                if (badge.getColor() != null) {
                    existingBadge.setColor(badge.getColor());
                }
